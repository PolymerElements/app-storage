...
AppIndexedDBMirrorWorker.prototype = {
  ...
  openDb: function() {
    this.__dbOpens =
        this.__dbOpens || new Promise(function(resolve, reject) {
          ...
          request.onupgradeneeded = function(event) {
            console.log(
                'Upgrade needed:', event.oldVersion, '=>', event.newVersion);
            var context = {
  ...
  operateOnStore: function(operation, storeName, mode, ...operationArgs) {
    ...
    return this.openDb().then(function(db) {
      console.log(
          'Store operation:', operation, storeName, mode, operationArgs);
  ...
  validateSession: function(session) {
    return Promise
        .all([this.openDb(), this.get(INTERNAL_STORE_NAME, 'session')])
    ...
    switch (event.data['type']) {
      ...
      case 'app-mirror-validate-session':
        this.validateSession(event.data['session']).then(function() {
          port.postMessage({'type': 'app-mirror-session-validated', 'id': id});
        });
        break;
