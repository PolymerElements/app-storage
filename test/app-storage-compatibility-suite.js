...
function testAppStorageDocumentCompatibility(extendedContext) {
  ...
  suite('storage key change', function() {
    ...
    test(
        'reinitializes stored value regardless of previous initialization success',
        function(done) {
          storage.initializeStoredValue =
  ...
  suite('app storage compatibility: <' + context.tagName + '>', function() {
    ...
    suite('syncing storage scenarios', function() {
      ...
      setup(function() {
        ...
        return Promise.all(
            [storageOne.transactionsComplete, storageTwo.transactionsComplete]);
      });
      ...
      test('syncs object values across elements', function() {
        ...
        return Promise
            ...
            .then(function(results) {
              var value = results[0];
              expect(value).to.be.equal(1);
              expect(storageOne.get('data.objectProperty'))
                  .to.be.eql(objectValue);
              expect(storageTwo.get('data.objectProperty'))
                  .to.be.eql(objectValue);
            });
      });
      ...
      test('syncs array values across elements', function() {
        ...
        return Promise
            ...
            .then(function(results) {
              var value = results[0];
              expect(value).to.be.equal(1);
              expect(storageOne.get('data.arrayProperty'))
                  .to.be.eql(arrayValue);
              expect(storageTwo.get('data.arrayProperty'))
                  .to.be.eql(arrayValue);
            });
      });
    });
